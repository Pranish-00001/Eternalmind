    ...
    >
      <div>
        ...
        <div className="flex items-center justify-between mb-4">
          <span className={`flex items-center gap-1.5 px-2.5 py-1 rounded text-caption font-mono uppercase tracking-wider border ${statusStyle.bg} ${statusStyle.border} ${statusStyle.text}`}>
        ...
        {/* Title */}
        <h3 className="text-subtitle-lg text-gray-100 group-hover:text-[#D4AF37] transition-colors uppercase tracking-wider mb-2">
          {capsule.title}
        </h3>

        {/* Description */}
        <p className="text-gray-400 text-body-sm line-clamp-2 leading-relaxed mb-6 font-light">
          {capsule.description || "No supplemental descriptions registered."}
        </p>

        {/* Core Metadata Stack */}
        <div className="space-y-2.5 border-t border-[#D4AF37]/10 pt-4 mb-6 font-mono text-caption text-gray-400">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-gray-300 font-mono text-caption" title={capsule.ownerWalletAddress}>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-[#D4AF37] font-bold text-caption">
              {new Date(capsule.unlockDate).toLocaleDateString()}
            </span>
          </div>

          {/* Attachments / Text count */}
          <div className="flex items-center justify-between pt-1 font-mono text-caption text-gray-500">
            <span>MEMORIES:</span>
      ...
      {isSealed && capsule.txHash && (
        <div className="mt-2 pt-3 border-t border-dashed border-[#D4AF37]/10 flex items-center justify-between bg-[#0E152D]/40 p-2 rounded">
          <div className="flex items-center gap-1 text-caption font-mono text-gray-500 truncate max-w-[80%]">
            <Hash className="w-3 h-3 text-emerald-400 flex-shrink-0" />
            <span className="uppercase text-caption mr-1">TXHASH:</span>
            <span className="text-gray-400 truncate">{capsule.txHash}</span>
          </div>
          <button 
            onClick={handleCopyTx}
            className="text-caption text-[#D4AF37] hover:text-white transition-colors cursor-pointer"
            title="Copy Tx Hash"
